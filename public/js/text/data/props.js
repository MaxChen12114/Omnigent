// js/text/data/props.js — 道具卡系统（6 类骨架机制：机制写死，内容用户填）
...
const TYPES={
  atmosphere:{label:"气氛卡",icon:"🌤️",svg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="8.5" cy="8.5" r="3"/><path d="M8.5 2.6v1.4M8.5 13v1.4M2.6 8.5h1.4M12.6 8.5h1.4M4.3 4.3l1 1M11.7 11.7l1 1M12.7 4.3l-1 1M5.3 11.7l-1 1"/><path d="M10 18.5h7a2.6 2.6 0 0 0 .2-5.2A3.6 3.6 0 0 0 10.6 13 2.6 2.6 0 0 0 10 18.5z"/></svg>',defaultDuration:3,durMin:1,durMax:10,allowDelta:false,maxActive:1,hint:"调对话气氛/语气，不直接动好感度",placeholder:"示例：气氛轻松愉快，可以多说日常闲聊。"},
  gift:      {label:"示好卡",icon:"🎁",svg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><rect x="3.5" y="9" width="17" height="11" rx="1.5"/><path d="M3 9h18v3.5H3z"/><path d="M12 9v11"/><path d="M12 9S10.6 4.5 8.2 4.5a2 2 0 0 0 0 4.5z"/><path d="M12 9s1.4-4.5 3.8-4.5a2 2 0 0 1 0 4.5z"/></svg>',defaultDuration:2,durMin:1,durMax:5, allowDelta:true,deltaMin:-5,deltaMax:5,defaultDelta:3,maxActive:99,hint:"一次性好感增减 + 短时气氛软化",placeholder:"示例：递出礼物，对方感到受宠若惊。"},
  roleplay:  {label:"角色扮演卡",icon:"🎭",svg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M5 5h14v5.5a7 7 0 0 1-14 0z"/><path d="M9 8.6c.6.5 1.5.5 2.1 0M12.9 8.6c.6.5 1.5.5 2.1 0"/><path d="M9.3 13.4c1.5 1.3 3.9 1.3 5.4 0"/></svg>',defaultDuration:5,durMin:1,durMax:20,allowDelta:false,maxActive:1,hint:"临时切换身份/关系设定",placeholder:"示例：假装互相不认识，重新建立关系。"},
  status:    {label:"状态卡",icon:"💧",svg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.2c3.4 4 5.8 6.9 5.8 9.9a5.8 5.8 0 0 1-11.6 0c0-3 2.4-5.9 5.8-9.9z"/><path d="M9.6 14.2a2.4 2.4 0 0 0 2.4 2.4"/></svg>',defaultDuration:4,durMin:1,durMax:10,allowDelta:false,maxActive:3,hint:"临时附加生理/精神状态",placeholder:"示例：微醺状态，话变多但不失智。"},
  rule:      {label:"规则卡",icon:"📜",svg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 3.5h8.5L18.5 7.5V20a.5.5 0 0 1-.5.5H6a.5.5 0 0 1-.5-.5V4a.5.5 0 0 1 .5-.5z"/><path d="M14 3.5V8h4.5"/><path d="M8.5 11.5h7M8.5 14.5h7M8.5 17.5h4"/></svg>',defaultDuration:3,durMin:1,durMax:10,allowDelta:false,maxActive:3,hint:"临时增加/修改对话规则",placeholder:"示例：必须诚实回答，不能回避问题。"},
  plot:      {label:"剧情卡",icon:"✨",svg:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M12 3.3l1.7 6L19.7 11l-6 1.7L12 18.7l-1.7-6L4.3 11l6-1.7z"/><path d="M18.6 14.4l.7 2.4 2.4.7-2.4.7-.7 2.4-.7-2.4-2.4-.7 2.4-.7z"/></svg>',defaultDuration:-1,durMin:-1,durMax:50,allowDelta:true,allowReset:true,deltaMin:-50,deltaMax:50,defaultDelta:10,maxActive:99,hint:"永久或重置型大事件（duration=-1 永久）",placeholder:"示例：失去对历史的所有记忆，从头开始。"},
};
...
// 2026-06-03: 6 类骨架图标去 emoji 改线性 SVG。卡片用默认图标(空或等于旧 emoji)→ 渲染类型 SVG；用户自填的 emoji 仍原样显示。
function icoHtml(icon,type){const t=TYPES[type]||TYPES.atmosphere;if(icon&&icon!==t.icon)return `<span class="prop-ico-emoji">${esc(icon)}</span>`;return `<span class="prop-ico-svg">${t.svg||""}</span>`;}
function injectIcoStyle(){if(document.getElementById("props-ico-style"))return;const s=document.createElement("style");s.id="props-ico-style";s.textContent=".prop-ico-svg{display:inline-flex;align-items:center;justify-content:center;line-height:0;}.prop-ico-svg svg{width:1em;height:1em;display:block;}.prop-ico-emoji{display:inline-flex;align-items:center;justify-content:center;line-height:1;}";document.head.appendChild(s);}
...
async function useProp(cardId){
  ...
  arr.push({cardId,type:c.type,name:c.name,icon:c.icon||"",systemInstruction:c.systemInstruction||"",durationLeft:durRaw});
  setActive(arr);
...
const open=()=>{ensure();injectIcoStyle();mask.style.display="flex";render();};
...
function activeSec(){
  ...
  for(const p of arr){
    ...
    h.push(`<div class="char-prop-active" style="display:flex;align-items:center;gap:8px;padding:6px 10px;border-radius:8px;border:1px solid;font-size:13px;"><span style="font-size:16px;">${icoHtml(p.icon,p.type)}</span><span style="flex:1;font-weight:600;">${esc(p.name)}</span><span style="font-size:11px;color:#888;">${esc(left)}</span><button class="char-rel-btn" data-remove="${esc(p.cardId)}" style="font-size:11px;padding:2px 8px;">卸下</button></div>`);
  }
  h.push(`</div>`);
  return h.join("");
}
...
function editor(b,card){
  ...
  const renderEditor=()=>{
    ...
    b.innerHTML=`<div class="char-edit-form">
      ...
      <select class="char-input" data-f="type">${Object.entries(TYPES).map(([k,v])=>`<option value="${k}" ${k===curType?"selected":""}>${v.label} — ${v.hint}</option>`).join("")}</select>
      <label class="char-label">卡名</label><input class="char-input" data-f="name" value="${esc(c.name)}" placeholder="自取（如：温柔的吻 / 续命咖啡 / 决裂宣言）">
      <label class="char-label">图标（可留空 = 用默认线性图标）</label><input class="char-input" data-f="icon" value="${esc(c.icon&&c.icon!==t.icon?c.icon:"")}" placeholder="留空=默认" maxlength="4" style="width:120px;">
    ...
    b.querySelector("[data-save]").addEventListener("click",async()=>{
      ...
      const s={id:c.id||("p_"+Date.now().toString(36)),type:curType,name,icon:g("icon").trim(),description:g("description").trim(),systemInstruction:si,duration:dur,exclusiveGroup:eg};/* icon 留空=用类型默认线性图标 */
...
// 4.61: 道具卡面板配色改用全站 CSS 变量(var(--bg)/--border/--bubble-ai/--input-bg…),
// 跟随四主题(minimal/glass/lewd-peach/lewd-doll)+「UI 配置」自定义覆盖。
// 旧版读已废弃的 my-theme(light/dark)硬编码 #0f0f0f/#fff,无视当前主题 → 永远老深色,看着像没更新。
function theme(){
  if(!P)return;
  const AC="var(--ue-accent, var(--btn-bg, #7d6cff))";
  P.style.background="var(--bg)";
  P.style.borderColor="var(--border)";
  P.style.color="inherit";
  P.querySelectorAll(".char-item").forEach(e=>{e.style.background="var(--bubble-ai, var(--input-bg, transparent))";e.style.borderColor="var(--border)";e.style.color="inherit";});
  P.querySelectorAll(".char-item.active").forEach(e=>{e.style.borderColor=AC;e.style.background="var(--bubble-ai, var(--input-bg, transparent))";});
  P.querySelectorAll(".char-rel-btn").forEach(e=>{e.style.background="transparent";e.style.color="inherit";e.style.opacity="0.72";e.style.borderColor="var(--border)";});
  P.querySelectorAll(".char-rel-btn.active").forEach(e=>{e.style.background=AC;e.style.color="#fff";e.style.opacity="1";e.style.borderColor="transparent";});
  // 4.62: 不再用内联 var() 覆盖输入框配色——交回 styles.css 的四主题 .char-input 规则。
  // 原内联 var(--input-bg) 会盖过正确的主题 CSS(尤其自定义 UI 配置下 --input-bg 为暗色时),导致浅色主题里输入框仍黑底。只清掉可能残留的旧内联值,让 CSS 接管。
  P.querySelectorAll(".char-input").forEach(e=>{e.style.removeProperty("background");e.style.removeProperty("color");e.style.removeProperty("border-color");});
  P.querySelectorAll(".char-section-title").forEach(e=>{e.style.color="var(--muted, currentColor)";});
  P.querySelectorAll(".char-prop-active").forEach(e=>{e.style.background="var(--bubble-ai, var(--input-bg, transparent))";e.style.borderColor="var(--border)";e.style.color="inherit";});
}
