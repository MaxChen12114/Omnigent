 ...
 *
 * ── 统一管线（image-common 接入版）──
 * 依赖 window.__imageCommon（image-common.js），须在本模块前加载。
 * 委托范围：getKey / ENDPOINTS 常量 / dlFetch（下载代理）。
 * 保留本模块特有语义：pollTask 返回 {status,raw} 状态对象（非抛错）、
 *   用户可调 steps/guidance/task_types、取消按钮 + 状态徽标、pollAbort 旗标。
 */
(function () {
  'use strict';
  if (window.__image) return;

  // ── 统一管线：底层鉴权 / 端点 / 下载代理委托 image-common.js ──
  var IC = window.__imageCommon;
  if (!IC) { console.error('[image-embed] 缺少 image-common.js，请确认其在本模块前加载'); return; }
  ...
  // 鉴权委托 image-common（读 cfw_image_key_v1，校验，抛错消息与原版一致）
  function getKey() { return IC.getKey(); }
  function nowTs() {
  ...
  // ── 网络：端点常量委托 image-common；/img/dl 代理亦委托（自动处理 data:/同源直传）──
  function apiFetch(path, opts) {
    opts = opts || {};
    var p = String(path).replace(/^\/+/, '');
    var url;
    if (p === 'images/generations') url = IC.ENDPOINTS.generations;
    else if (p === 'async/images/edits') url = IC.ENDPOINTS.editsAsync;
    else if (p.indexOf('task/') === 0) url = IC.ENDPOINTS.task + p.slice(5);
    else url = '/img/v1/' + p;
    return fetch(url, { method: opts.method || 'GET', headers: opts.headers || {}, body: opts.body || null, signal: opts.signal || null });
  }
  function dlFetch(url, opts) { return IC.dlFetch(url, opts); }
  ...
  function init() {
    bindSidebarEntry(); // 卡片由 settings.js 统一挂载
  }
  ...
  function ensureStyles() {
    ...
    s.textContent = [
      // 4.41-ui 工坊视觉重做:去掉生硬的 currentColor 描边,改柔和 rgba 边 + 紫色强调(#7d6cff,与思考按钮/右栏统一),输入框聚焦环,主操作整条紫色按钮
      '.img-mask{position:fixed;inset:0;z-index:40;display:none;align-items:flex-start;justify-content:center;background:rgba(0,0,0,.6);backdrop-filter:blur(4px);-webkit-backdrop-filter:blur(4px);overflow:auto;padding:32px 16px;}',
      '.img-mask.open{display:flex;}',
      '.img-studio{width:100%;max-width:680px;background:var(--bg,#1a1a1a);color:inherit;border:1px solid rgba(127,127,127,.22);border-radius:16px;padding:20px 20px 22px;box-shadow:0 18px 60px rgba(0,0,0,.55);}',
      '.img-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;}',
      '.img-title{font-weight:700;font-size:16px;letter-spacing:.01em;display:flex;align-items:center;gap:8px;}',
      '.img-badge-ver{font-size:11px;font-weight:500;opacity:.85;background:rgba(125,108,255,.14);color:#7d6cff;border:none;border-radius:20px;padding:2px 9px;margin-left:2px;}',
      '.img-close{background:transparent;border:1px solid rgba(127,127,127,.28);color:inherit;border-radius:9px;cursor:pointer;width:30px;height:30px;font-size:14px;opacity:.7;transition:opacity .15s,background .15s;}',
      '.img-close:hover{opacity:1;background:rgba(127,127,127,.14);}',
      '.img-tabs{display:flex;gap:4px;margin-bottom:16px;background:rgba(127,127,127,.10);border-radius:10px;padding:3px;}',
      '.img-tab{flex:1;padding:8px;border-radius:8px;border:none;background:transparent;color:inherit;cursor:pointer;font-size:13.5px;font-weight:600;opacity:.6;transition:opacity .15s,background .15s,box-shadow .15s;}',
      '.img-tab:hover{opacity:.85;}',
      '.img-tab.active{opacity:1;background:rgba(127,127,127,.22);box-shadow:0 1px 3px rgba(0,0,0,.2);}',
      '.img-lab{display:block;font-size:12px;opacity:.65;margin:12px 0 5px;font-weight:500;}',
      '.img-input,.img-textarea{width:100%;box-sizing:border-box;padding:9px 11px;border-radius:9px;border:1px solid rgba(127,127,127,.28);background:rgba(127,127,127,.07);color:inherit;font:inherit;transition:border-color .15s,box-shadow .15s;}',
      '.img-input:focus,.img-textarea:focus{outline:none;border-color:#7d6cff;box-shadow:0 0 0 3px rgba(125,108,255,.16);}',
      '.img-textarea{resize:vertical;min-height:80px;line-height:1.5;}',
      '.img-grid2{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:6px;}',
      '.img-checks{display:flex;flex-wrap:wrap;gap:8px;margin:6px 0;}',
      '.img-chk{font-size:12.5px;display:flex;align-items:center;gap:5px;cursor:pointer;padding:5px 10px;border-radius:7px;border:1px solid rgba(127,127,127,.25);transition:border-color .15s,background .15s;}',
      '.img-chk:hover{background:rgba(127,127,127,.10);}',
      '.img-row{margin-top:16px;}',
      '.img-btn{padding:9px 16px;border-radius:9px;border:1px solid rgba(127,127,127,.3);background:transparent;color:inherit;cursor:pointer;font-size:13px;opacity:.92;text-decoration:none;display:inline-block;transition:opacity .15s,background .15s,transform .1s;}',
      '.img-btn:hover{opacity:1;background:rgba(127,127,127,.12);}',
      '.img-btn:active{transform:translateY(1px);}',
      '.img-btn-primary{width:100%;text-align:center;border:none;background:#7d6cff;color:#fff;font-weight:600;font-size:14px;padding:11px 16px;opacity:1;box-shadow:0 2px 10px rgba(125,108,255,.35);}',
      '.img-btn-primary:hover{background:#8b7bff;}',
      '.img-btn-ghost{border-color:rgba(127,127,127,.28);font-size:12px;padding:5px 11px;}',
      '.img-statusbar{display:flex;align-items:center;gap:10px;margin:16px 0 6px;}',
      '.img-badge{font-size:12px;padding:5px 11px;border-radius:8px;border:1px solid rgba(127,127,127,.25);background:rgba(127,127,127,.06);opacity:.85;flex:1;}',
      '.img-badge-ok{border-color:rgba(37,194,160,.6);background:rgba(37,194,160,.10);}',
      '.img-badge-err{border-color:rgba(255,84,112,.6);background:rgba(255,84,112,.10);}',
      '.img-badge-info{border-color:rgba(125,108,255,.5);background:rgba(125,108,255,.10);}',
      '.img-output-head{display:flex;align-items:center;justify-content:space-between;font-size:12.5px;opacity:.65;font-weight:600;margin:12px 0 8px;}',
      '.img-output{display:flex;flex-direction:column;gap:10px;}',
      '.img-item{border:1px solid rgba(127,127,127,.18);border-radius:12px;padding:12px;background:rgba(127,127,127,.04);}',
      '.img-item h4{margin:0 0 4px;font-size:13px;}',
      '.img-item-meta{font-size:11px;opacity:.55;margin-bottom:8px;word-break:break-all;}',
      '.img-out-img{max-width:100%;border-radius:10px;display:block;}',
      '.img-item-row{margin-top:10px;display:flex;gap:8px;}',
      '@media(max-width:640px){.img-grid2{grid-template-columns:1fr;}.img-studio{padding:16px;border-radius:14px;}}'
    ].join('\n');
