...
// - Calls ai.gitee.com via same-origin proxy: /img/v1/* (image-routes.js) to avoid CORS.
// - Downloads images/videos via /img/dl?url=... (image-routes.js) to avoid cross-origin blocks.
...
function showPanel(model) {
  ...
  // panelWan21 仅存在于创作工坊(studio)页;若 app.js 被其它页复用且无此面板,空值保护避免抛错
  const _p21 = $("panelWan21");
  if (_p21) _p21.style.display = model === "Wan2.1-T2V-14B" ? "block" : "none";
}
...
async function apiFetch(path, {method="GET", headers={}, body=null, signal=null}={}) {
  const res = await fetch(`/img/v1/${path.replace(/^\/+/, "")}`, {
    method,
    headers,
    body,
    signal,
...
async function dlFetch(url, {signal=null}={}) {
  const u = `/img/dl?url=${encodeURIComponent(url)}`;
...
async function runHunyuanVideo() {
  ...
  addOutputItem({
    ...
    rawJson: j,
    openUrl: openAfter ? `/img/v1/task/${encodeURIComponent(taskId)}` : null,
  });
...
// 与 HunyuanVideo-1.5 同走异步文生视频链路 async/videos/generations（同 /img/v1 套壳代理 → worker → ai.gitee.com）。
// 仅 model 改为 Wan2.1-T2V-14B；字段沿用该端点已验证的 Hunyuan 形参（aspect_ratio / num_inferenece_steps(SIC) / num_frames / seed / fps）。
async function runWan21() {
  const apiKey = getApiKey();
  rememberKeyMaybe();

  const prompt = $("w21Prompt").value.trim();
  if (!prompt) throw new Error("请输入提示词 / Please input prompt");

  const negative_prompt = $("w21Neg").value.trim();
  const aspect_ratio = $("w21Aspect").value;
  const num_inferenece_steps = clampInt($("w21Steps").value, 1, 100, 30);
  const num_frames = clampInt($("w21Frames").value, 1, 300, 120);
  const fps = clampInt($("w21Fps").value, 1, 60, 24);

  const seedRaw = $("w21Seed").value;
  const seedInt = Number.parseInt(String(seedRaw), 10);
  const seed = (Number.isFinite(seedInt) && seedInt > 0) ? seedInt : null;

  const openAfter = $("w21OpenUrl").checked;
  ...
  setStatus("Wan2.1-T2V 创建任务... / Creating task...");
  const res = await apiFetch("async/videos/generations", {
    method: "POST",
    headers: {
      "Authorization": `Bearer ${apiKey}`,
  ...
  const j = await readJsonSafely(res);
  if (!res.ok) {
    setStatus("Wan2.1-T2V 失败 / Failed", "err");
    addOutputItem({
      title: "Wan2.1-T2V 创建任务失败 / Create task failed",
      meta: `HTTP ${res.status}`,
      rawJson: j,
    });
    throw new Error(`API 错误 / API Error (${res.status})`);
  ...
  if (!taskId) {
    setStatus("Wan2.1-T2V 失败 / Failed", "err");
    addOutputItem({
      title: "Wan2.1-T2V 未返回 task_id / Missing task_id",
      rawJson: j,
    });
    throw new Error("Task ID not found in response");
  }
  ...
  addOutputItem({
    title: "Wan2.1-T2V 任务已创建 / Task created",
    meta: `task_id=${taskId} • aspect_ratio=${aspect_ratio} • frames=${num_frames} • fps=${fps} • steps=${num_inferenece_steps}${seed !== null ? ` • seed=${seed}` : ""}`,
    rawJson: j,
    openUrl: openAfter ? `/img/v1/task/${encodeURIComponent(taskId)}` : null,
  });

  setStatus("Wan2.1-T2V 任务已创建，开始轮询...");
  const result = await pollTask(taskId, apiKey, {
    intervalMs: 10 * 1000,
    timeoutMs: 30 * 60 * 1000,
    onTick: (info) => {
      setStatus(waitingStatusText("Wan2.1-T2V", info.tick, info.elapsedMs));
    },
  });
  ...
  if (st !== "success") {
    setStatus(`Wan2.1-T2V ${st} / ${st}`, st === "failed" ? "err" : "info");
    addOutputItem({
      title: `Wan2.1-T2V 任务结束：${st} / Task ended: ${st}`,
      rawJson: raw,
      meta: `task_id=${taskId}`,
    });
    return;
  ...
  if (fileUrl) {
    ...
    addOutputItem({
      title: "Wan2.1-T2V 输出 / Output",
      meta: `task_id=${taskId} • file_url=${fileUrl}`,
      element: video,
      rawJson: raw,
      download: { href: blobInfo.objUrl, filename: `wan21-t2v-${nowTs()}.mp4` },
      openUrl: openAfter ? fileUrl : null,
    });

    setStatus("Wan2.1-T2V 成功 / Success", "ok");
  } else {
    addOutputItem({
      title: "Wan2.1-T2V 成功但无输出 / Success but no output",
      meta: `task_id=${taskId}`,
      rawJson: raw,
    });
    setStatus("Wan2.1-T2V 成功 / Success", "ok");
...
function initUi() {
  ...
  // Wan2.1-T2V-14B 执行按钮;空值保护:其它复用 app.js 但无此按钮的页面不会报错
  const _btnW21 = $("btnW21Run");
  if (_btnW21) _btnW21.onclick = async () => {
    try { await runWan21(); }
    catch (e) { addOutputItem({ title:"Wan2.1-T2V 错误 / Error", meta:String(e) }); }
  };
